#!/usr/bin/env python3
"""
Prompt Injection Checker - Detects prompt injection attacks in git changes.

Reads regex patterns from prompt-injection-config.json and scans changed files
in git for matches. Reports findings with file path, line number, and injection type.
"""
...
def load_patterns(config_path: str) -> List[Dict]:
    """Load prompt injection regex patterns from config file."""
    try:
...
def scan_file_for_injection(file_path: str, patterns: List[Dict]) -> List[Injection_Finding]:
    """Scan a file for prompt injection attempts."""
    findings = []
...
def print_findings(findings: List[Injection_Finding]) -> None:
    ...
    max_type_len = max(len(f.injection_type) for f in findings) if findings else 15
    max_context_len = 50
    ...
    for finding in findings:
        ...
        print(f"{finding.file:<{max_file_len}} | {finding.line_number:<6} | {finding.injection_type:<{max_type_len}} | {context:<{max_context_len}}")
...
def main():
    ...
    config_path = script_dir / 'prompt-injection-config.json'
    ...
    for file_path in changed_files:
        findings = scan_file_for_injection(file_path, patterns)
        all_findings.extend(findings)
    
    # Print results
    if all_findings:
        print(f"\n🚨 Found {len(all_findings)} potential prompt injection attack(s):\n")
        print_findings(all_findings)
        print(f"\nTotal findings: {len(all_findings)}")
        sys.exit(2)
    else:
        print("✅ No prompt injection attacks detected.")
        sys.exit(0)
